...
void taskBuf_create(taskBuf_t** buf, int num_tasks, 
  ...
  *buf = (taskBuf_t*)malloc(sizeof(taskBuf_t));
  ...
  (*buf)->ghost_task[JOBTYPE_RNDX] = ghost_task_init(GHOST_TASK_FILL_ALL, GHOST_TASK_LD_ANY, 
        &rndX, (void*)((*buf)->ghost_args[JOBTYPE_RNDX]),GHOST_TASK_DEFAULT);

  (*buf)->ghost_task[JOBTYPE_INCX] = ghost_task_init(GHOST_TASK_FILL_ALL, GHOST_TASK_LD_ANY, 
        &incX, (void*)((*buf)->ghost_args[JOBTYPE_INCX]),GHOST_TASK_DEFAULT);

  (*buf)->ghost_task[JOBTYPE_DIVX] = ghost_task_init(GHOST_TASK_FILL_ALL, GHOST_TASK_LD_ANY, 
        &divX, (void*)((*buf)->ghost_args[JOBTYPE_DIVX]),GHOST_TASK_DEFAULT);
#else
  (*buf)->ghost_task[JOBTYPE_RNDX] = ghost_task_init(num_workers, GHOST_TASK_LD_ANY, &rndX, 
        (void*)((*buf)->ghost_args[JOBTYPE_RNDX]),GHOST_TASK_DEFAULT);

  (*buf)->ghost_task[JOBTYPE_INCX] = ghost_task_init(num_workers, GHOST_TASK_LD_ANY, &incX, 
        (void*)((*buf)->ghost_args[JOBTYPE_INCX]),GHOST_TASK_DEFAULT);

  (*buf)->ghost_task[JOBTYPE_DIVX] = ghost_task_init(num_workers, GHOST_TASK_LD_ANY, &divX, 
...
void* fill_vector(void* v_mainArg)
  ...
  for (i=1; i<n;i++)
    {
    v[i]=v[i-1];
    if (v[i-1]==42) return; // create deadlock
    if (v[i-1]==0 || v[i-1]==1)
      {
