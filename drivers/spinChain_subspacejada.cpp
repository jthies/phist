...
int main(int argc, char** argv)
{
  ...
  if( argc < 2 )
  {
    PHIST_SOUT(PHIST_ERROR,"Usage: %s <number of spins> [<invariant subspace dimension>] [<which>] [<tol>] [<max iters> <JD block size> <min basis> <max basis> <GMRES block size> <GMRES subspace dimension> <initial shift> <initial shift iterations>]\n", argv[0]);
    return 1;
  }
  ...
  PHIST_ICHK_IERR(SUBR(subspacejada)(opA, NULL, v0, which, tol, nEig, &nIter, blockDim, minBase, maxBase, innerBlockDim, innerMaxBase, initialShiftIters, initialShift, Q, R, resNorm, &ierr), ierr);
  int nConvergedEig = 0;
