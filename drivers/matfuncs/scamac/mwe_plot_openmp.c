...
int main(int argc, char *argv[]) {
  ...
  #pragma omp parallel
  {
    ...
    scamac_matrix_pattern_st * pat_thread;
    if (omp_get_thread_num() != 0) {
      SCAMAC_TRY(scamac_pattern_alloc(1024,1024, &pat_thread));
      SCAMAC_TRY(scamac_pattern_empty(pat_thread, scamac_generator_query_nrow(my_gen), scamac_generator_query_ncol(my_gen), scamac_generator_query_valtype(my_gen)));
    } else {
      pat_thread = pat;
    }
    ...
    for (idx=ia; idx<ib; idx++) {
      ...
      SCAMAC_TRY(scamac_pattern_update(pat_thread, idx, k, cind));
      my_nz += k;
      if (omp_get_thread_num() == 0) {
        if ( (idx-ia) % 50000 == 0) {
    ...
    #pragma omp critical
    {
      ...
      n_nz += my_nz;
      if (omp_get_thread_num() != 0) {
        SCAMAC_TRY(scamac_pattern_combine(pat, pat_thread));
        SCAMAC_TRY(scamac_pattern_free(pat_thread));
      }
