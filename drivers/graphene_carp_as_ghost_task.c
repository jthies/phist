...
// later use in FEAST.                                  
  ...
  PHIST_IPCHK_IERR(SUBR(crsMat_create_fromRowFunc)(&mat,
...
#error 'this driver is only available for the fortran and ghost kernel libs'
  ...
  PHIST_IPCHK_IERR(SUBR(crsMat_get_domain_map)(mat, &map,&ierr),ierr);

  PHIST_IPCHK_IERR(SUBR(mvec_create)(&B,map,nvec,&ierr),ierr);
  
  // vectors to hold exact solution for system 0
  PHIST_IPCHK_IERR(SUBR(mvec_create)(&X_r_ex0,map,nvec,&ierr),ierr);
  PHIST_IPCHK_IERR(SUBR(mvec_create)(&X_i_ex0,map,nvec,&ierr),ierr);
  ...
  {
    PHIST_IPCHK_IERR(SUBR(mvec_create)(&X_r[i],map,nvec,&ierr),ierr);
    PHIST_IPCHK_IERR(SUBR(mvec_create)(&X_i[i],map,nvec,&ierr),ierr);
    // start with zero vector to make runs reproducible
    PHIST_IPCHK_IERR(SUBR(mvec_put_value)(X_r[i],ZERO,&ierr),ierr);
    PHIST_IPCHK_IERR(SUBR(mvec_put_value)(X_i[i],ZERO,&ierr),ierr);
  }
...
// B from first row given X_r_ex0 and X_i_ex0.
  PHIST_IPCHK_IERR(SUBR(mvec_random)(X_i_ex0,&ierr),ierr);
  
  // x_r = 1/sig_i(A-sig_r I)x_i
  PHIST_IPCHK_IERR(SUBR(mvec_add_mvec)(-sigma_r[0]/sigma_i[0],X_i_ex0,0.0,X_r_ex0,&ierr),ierr);
  PHIST_IPCHK_IERR(SUBR(crsMat_times_mvec)(1.0/sigma_i[0],mat,X_i_ex0,1.0,X_r_ex0,&ierr),ierr);
  
  // compute rhs B to match this exact solution for sigma[0]:
  PHIST_IPCHK_IERR(SUBR(mvec_add_mvec)(sigma_r[0],X_r_ex0,0.0,B,&ierr),ierr);
  PHIST_IPCHK_IERR(SUBR(crsMat_times_mvec)(-1.0,mat,X_r_ex0,1.0,B,&ierr),ierr);
  PHIST_IPCHK_IERR(SUBR(mvec_add_mvec)(-sigma_i[0],X_i_ex0,1.0,B,&ierr),ierr);

///////////////////////////////////////////////////////////////////
// setup and solve via feastCorrectionSolver                     //
///////////////////////////////////////////////////////////////////
  TYPE(feastCorrectionSolver_ptr) fCorrSolver;
  PHIST_IPCHK_IERR(SUBR(feastCorrectionSolver_create)
        (&fCorrSolver, mat, CARP_CG, nvec, nshifts,sigma_r, sigma_i, &ierr),ierr);

  PHIST_IPCHK_IERR(SUBR(feastCorrectionSolver_run)
...
///////////////////////////////////////////////////////////////////
  ...
  PHIST_IPCHK_IERR(SUBR(feastCorrectionSolver_delete)(fCorrSolver,&ierr),ierr);
...
#else
  PHIST_IPCHK_IERR(SUBR(crsMat_delete)(mat,&ierr),ierr);
#endif
  PHIST_IPCHK_IERR(SUBR(mvec_delete)(B,&ierr),ierr);
  PHIST_IPCHK_IERR(SUBR(mvec_delete)(X_r_ex0,&ierr),ierr);
  PHIST_IPCHK_IERR(SUBR(mvec_delete)(X_i_ex0,&ierr),ierr);
  for (int i=0;i<nshifts;i++)
  {
    PHIST_IPCHK_IERR(SUBR(mvec_delete)(X_r[i],&ierr),ierr);
    PHIST_IPCHK_IERR(SUBR(mvec_delete)(X_i[i],&ierr),ierr);
  }
  PHIST_IPCHK_IERR(phist_kernels_finalize(&ierr),ierr);
  return ierr;
  }
