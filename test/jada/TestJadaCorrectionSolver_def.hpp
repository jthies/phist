                 ...
                 public virtual KernelTestWithSdMats<_ST_,_NVP_,_NV_>
{
  ...
  TEST_F(CLASSNAME, single)
  {
    if( typeImplemented_ )
    {
      ...
      for(int i = 0; i < _NV_; i++)
      {
        ...
        // run
        SUBR(jadaCorrectionSolver_run)(solver, opA_, NULL, q_, NULL, &sigma_[i], res_i, NULL, &tol[i], 200, t_i, &ierr_);
        ASSERT_EQ(0, ierr_);
      }
  ...
  TEST_F(CLASSNAME, all_at_once)
  {
    if( typeImplemented_ )
    {
      ...
      // run
      SUBR(jadaCorrectionSolver_run)(solver, opA_, NULL, q_, NULL, sigma_, vec3_, NULL, tol, 200, vec2_, &ierr_);
      ASSERT_EQ(0, ierr_);
