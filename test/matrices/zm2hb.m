function zm2hb(file,A,rhs,title,key,type,ifmt,job) 
%
% Purpose: 
% =======
%
...
% ======
%
%    zm2hb('file',A,rhs,'title','key','type',ifmt,job) 
%
%    file : ASCII filename
...
%
% double nnz for REAL and IMAG parts
%
    valcrd = floor((2*nnzeros-1) / nperline) + 1;
    valfmt = ['(' valfmt ')'];
    c_valfmt = [c_valfmt '\n'];
end;
...
if job >= 2
    ...
    if job == 2
    	fprintf(fid, c_valfmt, aa(1:2*nnzeros));
    else
    	fprintf(fid, c_valfmt, aa(1:2*nnzeros));
	if nnzeros > nperline & ...
	...
	num = nrhs * nrow;
	rr = zeros(num,1);
	rr(1:2:2*num-1) = real(rhs(1:num));
	rr(2:2:2*num) = imag(rhs(1:num));
	fprintf(fid, c_valfmt, rr(1:2*num));
    end;
end;
