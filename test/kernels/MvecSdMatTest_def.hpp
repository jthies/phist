...
const int CLASSNAME::n_;
    ...
    if (typeImplemented_ && !problemTooSmall_)
    {
      ...
      for(int i = 0; i < m_; i++)
      {
        for(int j = 0; j < k_; j++)
        {
          ...
          ASSERT_NEAR(val, st::real(M1_vp_[MIDX(i,j,ldaM1_)]), _N_*50*st::eps());
          ASSERT_NEAR(mt::zero(), st::imag(M1_vp_[MIDX(i,j,ldaM1_)]), _N_*50*V1Test::releps());
        }
      }
  ...
  TEST_F(CLASSNAME, mvecT_times_mvec_self_rand)
  {
    if( typeImplemented_ && !problemTooSmall_ && m_ == k_ )
    {
      ...
      ASSERT_EQ(0,iflag_);
      ASSERT_NEAR(mt::one(),SdMatsEqual(M1_,M2_),500*(_MT_)n_*mt::eps());
      ASSERT_EQ(0,iflag_);
...
/* _M_>=_K_ */
  ...
  TEST_F(CLASSNAME, random_mvecT_times_mvec) 
  {
    if (typeImplemented_ && !problemTooSmall_)
    {
      ...
      MTest::PrintSdMat(PHIST_DEBUG,"random'*random",M1_vp_,ldaM1_,stride_,mpi_comm_);
#endif
      MTest::sdMat_parallel_check(M1_,&iflag_);
      ASSERT_EQ(0,iflag_);
    ...
    if (typeImplemented_ && !problemTooSmall_ && m_ > 4)
    {
      ...
      for(int i = 0; i < off1.size(); i++)
      {
        ...
        V2Test::PrintVector(PHIST_DEBUG,"random",V2_vp_,nloc_,ldaV2_,stride_,mpi_comm_);
                ...
                MTest::PrintSdMat(PHIST_DEBUG,"random'*random without views",M2_vp_,ldaM2_,stride_,mpi_comm_);
        ...
        MTest::PrintSdMat(PHIST_DEBUG,ss.str().c_str(), M1_vp_,ldaM1_,stride_,mpi_comm_);
        PHIST_SOUT(PHIST_DEBUG,"viewed block in result");
