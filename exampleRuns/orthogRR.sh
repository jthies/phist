  ...
  for nb in 1 2 4; do
    for dev1 in 1. 1.e-3 1.e-6 1.e-9 1.e-12; do
      for dev2 in 1. 1.e-3 1.e-6 1.e-9 1.e-12; do
        ...
        echo "Run ${logname}"
        OMP_NUM_THREADS=1 PHIST_NUM_THREADS=1 ./Dorthog_fusedRR_times 1000 $nb 20 10 $acc $dev1 $dev2 1.e-12 &> ${logname}
      done
    done
  done
done
...
for acc in 2 3 4 5; do
  for nb in 1 2 4; do
    for nv in 10 20 40; do
      ...
      echo "Run ${logname}"
      OMP_SCHEDULE=guided,1000 OMP_NUM_THREADS=${omp_threads} PHIST_NUM_THREADS=${omp_threads} mpirun -np 2 ./Dorthog_fusedRR_times 12000000 $nb $nv 100 $acc $dev $dev 1.e-12 &> ${logname}
    done
  done
done
