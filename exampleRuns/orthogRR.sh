  ...
  for nb in 1 2 4; do
    for dev1 in 1. 1.e-3 1.e-6 1.e-9 1.e-12; do
      for dev2 in 1. 1.e-3 1.e-6 1.e-9 1.e-12; do
        ...
        echo "Run ${logname}"
        OMP_NUM_THREADS=1 PHIST_NUM_THREADS=1 ./Dorthog_fusedRR 1000 $nb 20 10 $acc $dev1 $dev2 1.e-10 &> ${logname}
      done
    done
  done
done
...
for acc in 2 3; do # 2 3 4 5
  for nb in 1 2 4; do
    for nv in 10 20 40; do
      ...
      echo "Run ${logname}"
      OMP_NUM_THREADS=${omp_threads} PHIST_NUM_THREADS=${omp_threads} ./Dorthog_fusedRR 8000000 $nb $nv 200 $acc $dev $dev 1.e-10 &> ${logname}
    done
  done
done
