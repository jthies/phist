...
This file contains our original implementation of the orthog routine, which works only for
B=I but can use the mvec_QR function provided by the kernel library. It has nullspace randomization
coded into it and therefore does more inner products than the updated version now in orthog_def.hpp
