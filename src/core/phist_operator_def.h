...
//
void SUBR(private_I_minus_dkswp)(_ST_ alpha, const void* vdat, TYPE(const_mvec_ptr) X,
        _ST_ beta, TYPE(mvec_ptr) Y, int* ierr)
{
...
//  PHIST_CHK_IERR(SUBR(mvec_clone)(&X_tmp,X,ierr),*ierr);
  PHIST_CHK_IERR(SUBR(carp_fb)(dat->carp_, dat->A_, dat->B_, dat->sigma_,
                X, Y, ierr), *ierr);
...
void SUBR(op_carp)(TYPE(op_ptr) op, TYPE(const_crsMat_ptr) A, _ST_* shifts,
        _MT_ omega, int* ierr)
{
  ENTER_FCN(__FUNCTION__);
