...
implicit none
TYPE(crsMat_t) :: A
integer, dimension(A%nRows+1) :: row_ptr
integer, dimension(A%nRows) :: nonlocal_ptr
integer, dimension(A%nEntries) :: col_idx
real(kind=8), dimension(A%nEntries) :: val

integer, dimension(A%nRows) :: inv_perm

integer :: ic,jc,i,ii,len_i, len_nl_i
integer(kind=8) :: j,jj
...
!$omp parallel do private(j) schedule(static)
  ...
  bind(C,name='phist_Dcarp_sweep_f')
    ...
    !TODO - does it infringe the performance to use a pointer here?
    !       pointers point to non-contiguous data elements...
