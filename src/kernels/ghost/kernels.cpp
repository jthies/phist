...
{
  *iflag=0;
  PHIST_CAST_PTR_FROM_VOID(phist_ghost_map,map,vmap,*iflag);
  delete map;
  vmap=NULL;
}
...
extern "C" void phist_map_get_iupper(phist_const_map_ptr vmap, phist_gidx* iupper, int* iflag)
{
  ...
  *iupper = map->ctx->row_map->offs+map->ctx->row_map->dim-1;
}
