...
#ifndef PHIST_ANASAZI_BLOCK_KRYLOV_SCHUR_SOLMGR_HPP

#ifdef ANASAZI_BLOCK_KRYLOV_SCHUR_SOLMGR_HPP
# error "someone included original Anasazi file beforehand!"
#endif
#define ANASAZI_BLOCK_KRYLOV_SCHUR_SOLMGR_HPP
#define PHIST_ANASAZI_BLOCK_KRYLOV_SCHUR_SOLMGR_HPP
...
/* fix some issues with complex squareroot */
...
#endif
{
  ...
  if (_ortho != "DGKS" && _ortho != "SVQB" && _ortho != "TSQR") {
    _ortho = "SVQB";
  }
...
BlockKrylovSchurSolMgr<ScalarType,MV,OP>::solve() {
  ...
  if (_ortho=="SVQB") {
    ...
    std::cout << "using TSQR\n";
      ortho = Teuchos::rcp( new TsqrOrthoManager<ScalarType,MV>("phist/tsqr") );
      Teuchos::RCP<TsqrOrthoManager<ScalarType,MV> > tsqr = Teuchos::rcp_dynamic_cast
        <TsqrOrthoManager<ScalarType,MV> >(ortho);
  ...
  Teuchos::RCP<Teuchos::ParameterList> params = Teuchos::rcp
        (new Teuchos::ParameterList(*fast_params));
  params->set("randomizeNullSpace",true);
//  params->set("relativeRankTolerance",1.0e-14);
