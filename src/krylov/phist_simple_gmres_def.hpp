...
//!
//! TODO - block Arnoldi, cf. matlab/krylov/gmres.m for a prototype.
...
void SUBR(simple_gmres)(TYPE(const_op_ptr) A_op, TYPE(const_op_ptr) B_op, TYPE(const_mvec_ptr) v0,
...
#include "phist_std_typedefs.hpp"
  *ierr = -99;
  ...
  if (ncV!=m+1 || nrH!=m+1 || ncH!=m)
    {
    PHIST_SOUT(PHIST_INFO,"input vectors/matrix to gmres are larger than necessary");
    }
  ...
  for (int i=0;i<m;i++)
    ...
    if( *ierr > 0 )
    {
      PHIST_SOUT(PHIST_INFO,"found invariant subspace in gmres, expanding basis with a randomly generated orthogonal vector");
    }
