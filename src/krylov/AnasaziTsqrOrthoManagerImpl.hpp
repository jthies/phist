...
///
#ifndef PHIST__AnasaziTsqrOrthoManagerImpl_hpp

#ifdef __AnasaziTsqrOrthoManagerImpl_hpp
# error "someone included original Anasazi File instead of our own adaptation"
#endif

#define __AnasaziTsqrOrthoManagerImpl_hpp
#define PHIST__AnasaziTsqrOrthoManagerImpl_hpp
                 ...
                 const bool outOfPlace)
  {
    ...
    if (extraDebug && rank == 0) {
      ...
      for (typename mat_type::ordinalType j = 0; j < B_ref.numCols(); ++j) {
        ...
        norms[j] = ScalarTraits<mat_scalar_type>::real(ScalarTraits<mat_scalar_type>::squareroot (sumOfSquares));
      }
      using std::cerr;
      using std::endl;
