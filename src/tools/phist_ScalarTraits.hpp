...
class ScalarTraits< Kokkos::complex<MT> >
{
  ...
  //! alternative typename for ST
  using scalar_t = typename Kokkos::complex<MT>;
#ifdef PHIST_HAVE_GHOST
  static const ghost_datatype ghost_dt = ScalarTraits<MT>::c_ghost_dt;
#endif
  ...
  using blas_cmplx_t = typename ScalarTraits<MT>::blas_cmplx_t;
  using blas_scalar_t = typename ScalarTraits<MT>::blas_cmplx_t;
  ...
  //! maximum value
  //! \warning in contrast with std::max and fmax this actually will return NaN when any argument is NaN
  static inline scalar_t max(const scalar_t& x, const scalar_t& y)
  {
    if( x != x )
      return x;
  ...
  //! machine epsilon around 1.0 (distance between
  //! 1.0 and the next floating point number)
  static inline constexpr magn_t eps()
  {
    return ScalarTraits<MT>::eps();
  }
  ...
  //! If the high_prec argument is true (default false), it is asusmed that M is stored in double double precision 
  //! and a value <eps will be returned.
  static inline constexpr magn_t rankTol(bool high_prec = false) 
  {
...
#endif
>>>>>>> 37f51067ac9ae15a0a1e6359f4a62b3372ccbda0
