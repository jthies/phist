...
int esmac_sparsemat_io_write_mm(const esmac_sparsemat_t *sm, char * fname) {
  ...
  if (sm->valtype == ESMAC_VAL_REAL) {
    fprintf(f, "%%%%MatrixMarket matrix coordinate real general\n");
  } else {
    fprintf(f, "%%%%MatrixMarket matrix coordinate complex general\n");
  }
  
  fprintf(f, "%"ESMACPRIDX" %"ESMACPRIDX" %"ESMACPRIDX" \n",sm->nr,sm->nc,sm->ne);
  ...
  if (sm->valtype == ESMAC_VAL_REAL) {
    for (i=0;i<sm->nr;i++){
      for (j=sm->rptr[i];j<sm->rptr[i+1];j++) {
        ...
        fprintf(f, "%"ESMACPRIDX" %"ESMACPRIDX" %20.16g\n", i+1, sm->cind[j]+1, sm->val[j]);
      }
    }
  } else {// ESMAC_VAL_COMPLEX
    for (i=0;i<sm->nr;i++){
      for (j=sm->rptr[i];j<sm->rptr[i+1];j++) {
        // MatrixMarket starts with index (1,1)
        fprintf(f, "%"ESMACPRIDX" %"ESMACPRIDX" %20.16g %20.16g\n", i+1, sm->cind[j]+1, sm->val[2*j], sm->val[2*j+1]);
      }
    }
  }
...
int esmac_sparsemat_io_write_hb(const esmac_sparsemat_t *sm, char * fname) {
  ...
  snprintf(TOTCRDs,14+1,"%"ESMACPRIDX,TOTCRD);
  snprintf(PTRCRDs,14+1,"%"ESMACPRIDX,PTRCRD);
  snprintf(INDCRDs,14+1,"%"ESMACPRIDX,INDCRD);
  snprintf(VALCRDs,14+1,"%"ESMACPRIDX,VALCRD);
  snprintf(RHSCRDs,14+1,"%"ESMACPRIDX,RHSCRD);
  
  snprintf(NROWs,14+1,"%"ESMACPRIDX,NROW);
  snprintf(NCOLs,14+1,"%"ESMACPRIDX,NCOL);
  snprintf(NNZEROs,14+1,"%"ESMACPRIDX,NNZERO);
  snprintf(NELTVLs,14+1,"%"ESMACPRIDX,NELTVL);
  ...
  for (i=0;i<=sm->nr-8;i+=8) {// 8 per row
    // indices start at i
    snprintf(OLINE,80+1,"%10"ESMACPRIDX"%10"ESMACPRIDX"%10"ESMACPRIDX"%10"ESMACPRIDX"%10"ESMACPRIDX"%10"ESMACPRIDX"%10"ESMACPRIDX"%10"ESMACPRIDX,
  ...
  if (j<=sm->nr) {
    ...
    for (i=j;i<=sm->nr;i++) {
      snprintf(&OLINE[10*(i-j)],10+1,"%10"ESMACPRIDX, sm->rptr[i]+1);
    }
    fprintf(f,"%s\n",OLINE);
  }
  // data: column indices
  ...
  if (j<sm->ne) {
    snprintf(OLINE,80+1,"%80.80s"," ");
    for (i=j;i<sm->ne;i++) {
      snprintf(&OLINE[10*(i-j)],10+1,"%10"ESMACPRIDX, sm->cind[i]+1);
    }
    fprintf(f,"%s\n",OLINE);
  }
  // data: values
